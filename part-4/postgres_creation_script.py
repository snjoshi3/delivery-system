...
def create_sql_function_vanilla(conn):
...
def create_sql_function_for_update_no_skip(conn):
    # todo: add more sql procedures for without partition cluse and with partition clause, also without skip locked and with skip locked
    cursor = conn.cursor()
    cursor.execute("""
        CREATE OR REPLACE FUNCTION update_inventory(order_id_param INT, med_id_param INT, zip_code_param INT, quantity_param INT)
...
def create_sql_function_for_update_skip_locked(conn):
    # todo: add more sql procedures for without partition cluse and with partition clause, also without skip locked and with skip locked
    cursor = conn.cursor()
    cursor.execute("""
        CREATE OR REPLACE FUNCTION update_inventory(order_id_param INT, med_id_param INT, zip_code_param INT, quantity_param INT)
...
def create(type):
    ...
    try:
        ...
        if type == 'vanilla':
            create_sql_function_vanilla(conn)
        elif type == 'forupdatenoskip':
            create_sql_function_for_update_no_skip(conn)
